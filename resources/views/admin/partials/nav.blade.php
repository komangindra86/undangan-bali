    ...
    <div class="max-w-7xl mx-auto flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-col gap-3 md:flex-row md:items-center">
            <nav class="flex flex-wrap gap-2 text-sm">
                ...
                @php($openReports = \App\Models\ContentReport::where('status', 'open')->count())
                <a href="{{ route('admin.reports.index') }}" class="rounded-full border px-4 py-2 {{ request()->routeIs('admin.reports.*') ? 'border-amber-400 bg-amber-400 text-stone-950 font-semibold' : 'border-stone-700 text-stone-300 hover:border-amber-400' }}">Laporan{{ $openReports ? ' ('.$openReports.')' : '' }}</a>
